...
div_df <- create_richness_df(rich_dir, div_dir, zones_lu_fp, combo_ras, overwrite = FALSE)
div_df <- div_df %>% 
...
fn <- str_c('grpdiffs_', fn_prefix, '_', by_biom_code, var2_code, grp_var_code, '.rds')
...
grp_var_code <- str_remove_all(grp_var, '_.*')
fn <- str_c('grpdiffs_', fn_prefix, '_', 
...
if(!file.exists(fig_fp)){ 
  ggsave(fig_fp, width=19, height=10, units = 'cm', dpi = 600)
  }
...
# Save figure
fig_fp <- str_c(tools::file_path_sans_ext(grp_diffs_fp), '_beesnesting.png')
if(!file.exists(fig_fp)) {
  ggsave(fig_fp, p_bynesting, width=19, height=7, units = 'cm', dpi = 600)
  }
...
# Save figure
fig_fp <- str_c(tools::file_path_sans_ext(grp_diffs_fp), '_beessocial.png')
if(!file.exists(fig_fp)) {
  ggsave(fig_fp, p_bysocial, width=19, height=6, units = 'cm', dpi = 600)
}

# Put the plots together
plots <- list(p_bysocial, 
              p_bynesting)
...
if(!file.exists(fig_fp)) ggsave(fig_fp, width=19, height=10, units = 'cm', dpi = 600)
```
