...
extract_stats_to_csv <- function(rich_tif_fp, div_dir, combo_ras, 
                                 rich_dir = NA, col_types = 'ifffdf', overwrite = FALSE){
...
create_richness_df_with_ids <- function(rich_dir, div_dir, zones_lu_fp, combo_ras, overwrite = FALSE) {
  
  df_fp <- file.path(rich_dir, 'richness_df_with_ids.rds')
  if(file.exists(df_fp) & !overwrite) return(readRDS(df_fp))
  ...
  div_df <- fps %>% purrr::map_dfr(extract_stats_to_csv, div_dir, combo_ras,
                                   rich_dir = div_dir, col_type = 'iffffdf', overwrite= FALSE)
...
div_dir <- "data/data_out/sdm/rf4_excl_unqc_alldates_LcAltWc/diversity_by_zones/anp_ids"
anp_id_df <- create_richness_df_with_ids(rich_dir, div_dir, zones_lu_fp, combo_ras, overwrite = FALSE)
anp_id_df <- anp_id_df %>% 
  mutate(pol_group = factor(pol_group, 
                            levels = c('bee', 'butterfly', 'moth', 
                                       'bat', 'hummingbird', 'fly', 'wasp')))
